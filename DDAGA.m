...
OutputMoJo=strcat(DirName,'MoJo\Mozilla_accessible_DDAGA_',num2str(OutputNumber),'.rsf');
OutputPreRecall=strcat(DirName,'PreRecall\BestPop_DDAGA_',num2str(OutputNumber));

FigureDir=strcat(DirName,'Figure\DDAGA\',num2str(OutputNumber));
mkdir(FigureDir);
...
pop=CalaulatePopulationFitness(GraphDependency,pop,FileNumber);
...
for iter=1:maxiter
  ...
  [popcrossmut]=[crosspop,mutpop];
   ...
   popcrossmut=CalaulatePopulationFitness(GraphDependency,popcrossmut,FileNumber);
   [pop]=[pop,popcrossmut];
